		...
		for k,v in self.data.items():
			...
			if cores:
				...
				for core in cores: 
					...
					threads[-1].start()
					threads[-2].start() 
					self.pool.append(threads[-1])
					self.pool.append(threads[-2])  
					i=i+1
			...
			if(v['OS'] == "w"):
				...
				g6 = trendGraph(k+"/storage",'Total','On use','','Almacenamiento',20,50,60) 
			else:
				createRDD(k+"/storage",4,"GAUGE")
				u6 = updateRDD(k+"/storage",v['community'],v['hostname'],'1.3.6.1.2.1.25.2.3.1.4.36','1.3.6.1.2.1.25.2.3.1.5.36','1.3.6.1.2.1.25.2.3.1.4.36','1.3.6.1.2.1.25.2.3.1.6.36',v['port'])
				#special tag
				g6 = trendGraph(k+"/storage",'Total','On use','','Almacenamiento',20,50,60)
			...
			if(v['OS'] == "w"):
				...
				u7 = updateRDD(k+"/ram",v['community'],v['hostname'],'1.3.6.1.2.1.25.2.3.1.4.3','1.3.6.1.2.1.25.2.3.1.5.3','1.3.6.1.2.1.25.2.3.1.4.3','1.3.6.1.2.1.25.2.3.1.6.3',v['port'])
				g7 = trendGraph(k+"/ram",'Total','On use','','RAM en uso',20,50,60)

			else:
				createRDD(k+"/ram",4,"GAUGE")
				u7 = updateRDD(k+"/ram",v['community'],v['hostname'],'1.3.6.1.2.1.25.2.3.1.4.1','1.3.6.1.2.1.25.2.3.1.5.1','1.3.6.1.2.1.25.2.3.1.4.1','1.3.6.1.2.1.25.2.3.1.6.1',v['port'])
				#special tag
				g7 = trendGraph(k+"/ram",'Total','On use','','RAM en uso',20,50,60)
